...
function startAnimation() {
  ...
  const step =
    (currentPoints[currentPoints.length - 1].x - currentPoints[0].x) / 100;
  ...
  if (method === "mnk") {
    ...
    if (degree >= currentPoints.length) {
      alert(
        `Помилка! Степінь полінома (${degree}) має бути меншою за кількість точок (${currentPoints.length}).`,
      );
      return;
    }
  ...
  function animateCurve() {
    ...
    let calcY =
      method === "lagrange"
        ? getLagrangeValue(currentX, currentPoints)
        : getLSMValue(currentX, coeffs);

    if (currentX === currentPoints[0].x)
      ctx.moveTo(toScreenX(currentX), toScreenY(calcY));
